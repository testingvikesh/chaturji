    ...
    <form method="POST"
          ...
          x-data="{
              ...
              onPick(event) {
                  ...
                  this.files = picked.slice(0, 5).map(file => ({ name: file.name, size: file.size }));
                  if (picked.length > 5) {
        ...
        @csrf
        <input type="hidden" name="ticket_type" value="issue">

        <div class="mb-1">
            <h3 class="text-base font-bold text-slate-900">Issue Ticket</h3>
            <p class="text-xs text-slate-500 mt-0.5">Fill the form below and submit your issue.</p>
        </div>
        ...
        <div>
            ...
            <p class="text-xs text-slate-500 mb-2">Up to 5 files · max 10 MB each · JPG, PNG, WEBP, PDF, DOC, DOCX, TXT, ZIP</p>
            <input type="file"
                   name="attachments[]"
                   x-ref="fileInput"
                   multiple
            ...
            <div x-show="files.length" x-cloak class="mt-3 rounded-xl border border-slate-200 bg-slate-50 p-3">
                ...
                <ul class="space-y-1.5">
                    <template x-for="(file, index) in files" :key="'issue-'+index">
        ...
        <div class="flex items-center gap-3 pt-2">
            <button type="submit" class="admin-btn-primary" :disabled="sending" :class="{ 'opacity-60 cursor-not-allowed': sending }">
                <span x-text="sending ? 'Submitting…' : 'Generate Issue Ticket'">Generate Issue Ticket</span>
            </button>
