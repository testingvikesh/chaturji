            ...
            <div class="login-direct-form-card">
                <div class="lg:hidden flex items-center gap-3 mb-6">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-500">{{ $roleLabel }} Login</p>
                    </div>
                </div>

                <span class="section-badge">{{ $roleLabel }} Login</span>
                <h1 class="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight mt-1">Welcome Back</h1>
                <p class="text-sm text-slate-500 mt-2 mb-6">
                    @if ($role === 'teacher')
                        Sign in with your password or today's 4-digit OTP
                    @else
                ...
                <form method="POST" action="{{ route($role . '.login') }}" class="space-y-4">
                    @csrf
                    ...
                    <div>
                        <div class="flex items-center justify-between gap-3">
                            <label for="password" class="auth-label">{{ $role === 'teacher' ? 'Password or 4-digit OTP' : 'Password' }}</label>
                            <a href="{{ route('password.request', ['role' => $role]) }}" class="text-xs font-semibold text-brand-green hover:underline">Forgot password?</a>
                        </div>
                        <x-password-input id="password" name="password" required autocomplete="current-password" class="auth-input" placeholder="{{ $role === 'teacher' ? 'Password or today\'s 4-digit OTP' : 'Enter your password' }}" />
                    ...
                    <button type="submit" class="auth-submit login-direct-submit">Sign In</button>
                </form>
                ...
                <p class="text-center text-sm text-slate-500">
                    ...
                    <a href="{{ route($role . '.register') }}" class="auth-link">Register here</a>
                </p>
