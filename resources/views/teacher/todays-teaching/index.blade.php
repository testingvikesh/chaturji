<x-teacher-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            ...
            <a
                href="{{ route('teacher.todays-teaching.create', ['date' => $date]) }}"
    ...
    <div class="admin-page space-y-6">
        ...
        <form method="GET" action="{{ route('teacher.todays-teaching.index') }}" class="admin-card p-4 sm:p-5">
        ...
        <div class="grid sm:grid-cols-2 xl:grid-cols-4 gap-4">
            ...
            @include('admin.partials.stat-card', ['label' => 'Homework Generated', 'value' => $stats['homework_generated']])
        </div>

        <div class="admin-card overflow-hidden">
            <form id="generate-teaching-homework" method="POST" action="{{ route('teacher.todays-teaching.generate-homework') }}" class="hidden" aria-hidden="true">
                @csrf
            </form>

            @if ($logs->where('status', 'completed')->whereNull('homework_id')->isNotEmpty())
                <div class="border-b border-slate-100 bg-brand-green-50/50 px-4 py-4 sm:px-6 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                    <div>
                        <h3 class="font-bold text-slate-900 text-sm">Generate Homework</h3>
                        <p class="text-xs text-slate-500">Select completed rows, then choose marks to preview homework</p>
                    </div>
                    <div class="flex flex-wrap gap-2">
                        @foreach ($markOptions as $marks)
                            <button
                                type="submit"
                                form="generate-teaching-homework"
                                name="target_marks"
                                value="{{ $marks }}"
                                ...
                                onclick="return document.querySelectorAll('[form=generate-teaching-homework]:checked').length > 0 || (alert('Please select at least one completed topic.'), false)"
                            >
                                {{ $marks }} Marks
                            </button>
                        @endforeach
            ...
            <div class="admin-card-header border-b-0 pb-0">
                <h3 class="font-bold text-slate-900">Teaching Report — {{ \Carbon\Carbon::parse($date)->format('d M Y') }}</h3>
            </div>
                ...
                <table class="admin-table w-full text-sm">
                    ...
                    <tbody>
                        @forelse ($logs as $log)
                            <tr>
                                <td>
                                    @if ($log->isCompleted() && ! $log->hasHomework())
                                        <input type="checkbox" form="generate-teaching-homework" name="teaching_log_ids[]" value="{{ $log->id }}" class="rounded border-slate-300 text-brand-green">
                                    @endif
                                </td>
                                    ...
                                    <td>
                                        @if ($log->homework)
                                            <a href="{{ route('teacher.homework.show', $log->homework) }}" class="text-brand-green font-semibold hover:underline text-xs">
                                    ...
                                    <td class="whitespace-nowrap space-x-2">
                                        @if (! $log->hasHomework())
                                            @if ($log->isRemaining())
                                                <form method="POST" action="{{ route('teacher.todays-teaching.status', $log) }}" class="inline">
                                                    @csrf
                                                    @method('PATCH')
                                                    <input type="hidden" name="status" value="completed">
                                                    <input type="hidden" name="target_marks" value="20">
                                                    <button type="submit" class="text-xs font-semibold text-brand-green hover:underline" title="Marks homework 20 by default">Mark Completed</button>
                                                </form>
                                            @else
                                                <form method="POST" action="{{ route('teacher.todays-teaching.status', $log) }}" class="inline">
                                                    @csrf
                                        ...
                                        <form
                                            method="POST"
                                            action="{{ route('teacher.todays-teaching.destroy', $log) }}"
                                            class="inline"
                                            onsubmit="return confirm(@js($log->hasHomework()
                                                ? 'Remove this topic from today\'s report? Homework will stay in My Homework.'
                                                : 'Remove this teaching entry?'))"
                                        >
                                            @csrf
                                            @method('DELETE')
                            ...
                            @empty
                                <tr>
                                    <td colspan="8" class="text-center py-12 text-slate-500">
                                        No teaching entries for this date. Click <strong>+ Add Topics</strong> to start.
                                    </td>
                                </tr>
                        @endforelse
                    </tbody>
