<x-teacher-layout>
    <x-slot name="header">
        <div>
            <a href="{{ route('teacher.todays-teaching.index', ['date' => $date]) }}" class="text-xs font-medium text-brand-green hover:underline mb-1 inline-block">&larr; Back to Today's Teaching</a>
            <h2 class="admin-page-title">Add Teaching Topics</h2>
            <p class="admin-page-subtitle">Select chapter → topics → set marks & question types → preview homework</p>
        </div>
    </x-slot>

    <form
        method="POST"
        action="{{ route('teacher.todays-teaching.store') }}"
        ...
        x-on:submit="
            ...
            if (hasCompleted() && !document.querySelector('input[name=target_marks]:checked')) { $event.preventDefault(); alert('Please select homework marks for completed topics.'); }
        "
    >
        @csrf
        ...
        <div class="admin-card p-6 space-y-4">
            ...
            <div class="grid sm:grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1">Teaching Date *</label>
                    <input type="date" name="teaching_date" value="{{ old('teaching_date', $date) }}" required class="w-full rounded-xl border-slate-200">
                </div>
                <div>
        ...
        <div class="admin-card p-6" x-show="chapterId" x-cloak>
            <div class="flex items-center justify-between gap-4 mb-4">
                <div>
                    ...
                    <p class="text-xs text-slate-500 mt-1">Completed = preview homework · Remaining = report only</p>
                </div>
        ...
        <div class="admin-card p-6 border-brand-green/30 bg-brand-green-50/40" x-show="hasCompleted()" x-cloak>
            <h3 class="font-bold text-slate-900">Homework Paper</h3>
        ...
        <div class="flex flex-wrap gap-3">
            <button type="submit" class="rounded-xl bg-brand-green text-white px-6 py-2.5 text-sm font-semibold hover:bg-brand-green-dark transition">
                <span x-show="hasCompleted()">Preview Homework</span>
                <span x-show="!hasCompleted()" x-cloak>Add to Report</span>
            </button>
            <a href="{{ route('teacher.todays-teaching.index', ['date' => $date]) }}" class="rounded-xl border border-slate-200 px-6 py-2.5 text-sm font-semibold text-slate-600 hover:bg-slate-50 transition">Cancel</a>
        </div>
    </form>
</x-teacher-layout>
